<?php
// filepath: app/Services/OpenRouterOcrService.php
...
class OpenRouterOcrService implements OcrAnalyzerInterface
{
    public function analyzeMatchImage(UploadedFile $image, array $context, int $homeId, int $awayId): array
    {
        ...
        $response = Http::withHeaders([
            ...
            'HTTP-Referer' => env('APP_URL', 'https://superligafootballmanager.com'), // Requerido por OpenRouter
            'X-Title' => 'Superliga FM OCR', // Opcional, nombre de tu app
        ...
        if (!$response->successful()) {
            throw new Exception("OpenRouter HTTP Error: " . $response->body());
        }

        $responseData = $response->json();

        // OpenRouter a veces devuelve errores en la estructura si el modelo gratuito se cae
        if (isset($responseData['error'])) {
            throw new Exception("Error interno de OpenRouter: " . json_encode($responseData['error']));
        }
        ...
        if (!$data) {
            throw new Exception('OpenRouter no pudo generar un JSON válido. Respuesta cruda: ' . $text);
        }
